...
function ENT:SetMeteorTarget(ent)
    ...
    local VNormal = (Vector(ent:GetPos().x + math.random(-200,200),ent:GetPos().y + math.random(-200,200),ent:GetPos().z) - self:GetPos()):GetNormal()
    self:GetPhysicsObject():ApplyForceCenter(VNormal * speed)
end

function ENT:Destruct(notexplode)
    if not notexplode then
        util.BlastDamage(self, self, self:GetPos(), 500, 60)
    end
