...
rb655_AddForcePower({
	...
	action = function(self)
		...
		for id, ent in pairs(self:SelectTargets(3)) do
			...
			timer.Create("Freeze Timer" .. ent:EntIndex(), 10, 1, function()
				if IsValid(ent) then
					ent:Freeze(false)
				end
			end)
...
rb655_AddForcePower({
	...
	action = function(self)
		...
		if (foundents > 0) then
			...
			if (not self.SoundLightning) then
...
rb655_AddForcePower({
	...
	action = function(self)
		if (self:GetForce() < 10 or CLIENT) then return end
		local foundents = 0
...
rb655_AddForcePower({
	...
	action = function(self)
		if (self:GetForce() < 30 or CLIENT) then return end

		for id, ent in pairs(self:SelectTargets(1)) do
			if (not IsValid(ent) or ent.Chocked) then continue end
			if not ent:IsPlayer() then continue end
			ent.Chocked = true
			ent:Freeze(true)
			local elev = 100
			...
			timer.Create("star_choke_" .. ent:EntIndex(), time + 3, 1, function()
				if (not IsValid(ent)) then return end
				local dmg = DamageInfo()
				ent:Freeze(false)
				dmg:SetAttacker(self.Owner or self)
...
rb655_AddForcePower({
	...
	action = function(self)
		if (self:GetForce() < 60 or CLIENT or not self.Owner:IsOnGround()) then return end
		local elev = 400
		local time = 1
		ent = self.Owner
		...
		timer.Create("test", 0.6, 1, function()
			if (self.SoundLightning) then
				self.SoundLightning:Stop()
				self.SoundLightning = nil
			end
	...
	action = function(self)
		...
		for x = 1, 4 do
			...
			for x, y in pairs(ents.FindInSphere(org, 512)) do
				if (y:IsPlayer() or y:IsNPC()) and y ~= self.Owner and not Hit[y:EntIndex()] then
					Hit[y:EntIndex()] = y
					ent = y
				end
			end
...
rb655_AddForcePower({
	...
	action = function(self)
		...
		if (self:GetForce() < 10 or not trace.Entity:IsDoor() or CLIENT) then return end
		self:SetForce(self:GetForce() - 10)
		self:SetNextAttack(1)
...
-- DON"T TOUCH
local posv = {pi * 2 / 5, pi * 4 / 5, pi * 6 / 5, pi * 8 / 5, 2 * pi,}
